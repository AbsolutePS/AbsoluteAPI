...
This endpoint is used for getting device data from Absolute.
...
This endpoint is used for getting SIEM events generated by Absolute.

#### SiemEvents Return
The method GetSiemEvents<T>(RequestConfiguration configuration) is available but must use either the SiemEvents class as the return type (the default, pre-defined class) or a class that inherits from SiemEvents in case you want to use a custom class for the return type.

Example of this method:

    NameValueCollection parameters = new NameValueCollection();
    parameters.Add("fromDate", "2020-01-01T00:00:00.000Z");
    parameters.Add("toDate", "2020-12-31T00:00:00.000Z");
    parameters.Add("limit", "10");
    var requestConfig = new RequestConfiguration().WithParameters(parameters).WithOData(false);
    
    try
    {
        var siemEvents = await _api.SiemEvents.GetSiemEvents<SiemEvents>(requestConfig);

        Assert.True(siemEvents != null);
    }
    catch (WebException webException)
    {
        var responseBody = HttpHelper.ReadResponseBody(webException.Response);
        Assert.True(false, responseBody);
    }
    catch (Exception ex)
    {
...
##### Inheriting from SiemEvents
We can inherit from SiemEvents to get all the default fields and an additional field if it wasn't present on the SiemEvents class:

    class TestClass : SiemEvents
    {
        public string Whatever { get; set; }
    }

The inherited class can be used just like in previous example of code but with the custom type instead, as seen below with GetSiemEvents<T>:

    fields = await _api.SiemEvents.GetSiemEvents<TestClass>(requestConfig);

#### Dynamic return
The method GetSiemEvents(RequestConfiguration configuration) is available but the caller must accept return type of dynamic. This can be easier or harder to deal with depending on your familiarity with the dynamic features in C#.

Example of this method:

    NameValueCollection parameters = new NameValueCollection();
    parameters.Add("fromDate", "2020-02-01T00:00:00.000Z");
    parameters.Add("toDate", "2020-02-28T00:00:00.000Z");
    parameters.Add("limit", "10");
    var requestConfig = new RequestConfiguration().WithParameters(parameters).WithOData(false);

    try
    {
        var siemEvents = await _api.SiemEvents.GetSiemEventsDynamic(requestConfig);
          
        Assert.True((siemEvents as JObject).Count > 0);
    }
    catch (WebException webException)
    {
        var responseBody = HttpHelper.ReadResponseBody(webException.Response);
...
### Unenroll Endpoint V2

This endpoint is used for submitting agent removals to Absolute to retire or disable devices in the Absolute system.

This endpoint currently does not use a default class.  You must evaluate the response and create your own custom class if you don't want to use dynamic.

#### Dynamic return
The method UnenrollDevices<T>(IEnumerable<Guid> deviceGuids) is available for any type but the example we have uses dynamic only.

Example of this method:
    
    // Note that "device" can be any class or object that contains the Device ID
    // Most often, this is a ReportingDevice that was pulled from the Reporting Devices Endpoint's default return class
    var guid = device.Id;

    // Note that the List<Guid> could contain more than one ID. We recommend limiting this to batches of 1,000 or less
    var result = _api.UnenrollEndpointV2.UnenrollDevices<dynamic>(new List<Guid>() { (Guid)guid }).Result;
